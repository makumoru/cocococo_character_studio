...
class EditorWindow(EditorDNDWindow):
    ...
    def _execute_share(self):
        """保存、ZIP作成、API通信を連続して行うワーカースレッド"""
        try:
            # ワーカースレッドの先頭で、UIからのデータ収集と保存処理を行う
            # これにより、UIスレッドをブロックせずに安全に保存できる
            self.collect_data_from_ui()
            self.character_data.save()
            
            # 以降の処理は、保存された最新のファイルに対して行われる
            pat = self.github_uploader.get_pat()
            if not pat:
                # エラーメッセージではなく、トークンエラー専用のフラグを立てる
            ...
            # ZIP化の際の名前には、引き続きCHARACTER_NAMEかプロジェクトIDを使う
            # 戻り値をタプルで受け取る
            ...
            # 共有対象のIssue参照を確認（[GITHUB] ISSUE_NUMBER / ISSUE_URL）
            ...
            # censored_thumbnail_path を成功時のコールバックに渡す
