...
class TabFavorability(TabBase):
    def create_widgets(self):
        ...
        heart_ui_frame = ttk.Frame(hearts_frame, padding=(0, self.app.padding_normal, 0, 0))
        heart_ui_frame.grid(row=2, column=0, sticky="ew")
        
        ttk.Label(heart_ui_frame, text="（全てのハートで共通の設定です）", font=self.app.font_small).grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, self.app.padding_small))
        
        ui_pady = self.app.padding_small
        ttk.Label(heart_ui_frame, text="透過色:", font=self.app.font_normal).grid(row=1, column=0, sticky="w")
        trans_color_preview = tk.Label(heart_ui_frame, text="      ", relief="solid", bd=self.app.border_width_normal)
        trans_color_preview.grid(row=1, column=1, sticky="w", padx=ui_pady)
        ttk.Button(heart_ui_frame, text="色の選択", command=lambda: self.editor.pick_color(trans_color_preview), style="Tab.TButton").grid(row=1, column=2, padx=ui_pady)
        ttk.Button(heart_ui_frame, text="画像から色を取得", command=lambda: self.editor.enter_eyedropper_mode(trans_color_preview), style="Tab.TButton").grid(row=1, column=3, padx=ui_pady)
        
        ttk.Label(heart_ui_frame, text="縁色:", font=self.app.font_normal).grid(row=2, column=0, sticky="w", pady=(ui_pady,0))
        edge_color_preview = tk.Label(heart_ui_frame, text="      ", relief="solid", bd=self.app.border_width_normal)
        edge_color_preview.grid(row=2, column=1, sticky="w", padx=ui_pady, pady=(ui_pady,0))
        ttk.Button(heart_ui_frame, text="色の選択", command=lambda: self.editor.pick_color(edge_color_preview), style="Tab.TButton").grid(row=2, column=2, padx=ui_pady, pady=(ui_pady,0))
        ttk.Button(heart_ui_frame, text="画像から色を取得", command=lambda: self.editor.enter_eyedropper_mode(edge_color_preview), style="Tab.TButton").grid(row=2, column=3, padx=ui_pady, pady=(ui_pady,0))

        self.widgets = {'stages_tree': self.stages_tree, 'hearts_tree': self.hearts_tree, 'HEART_TRANSPARENT_COLOR': trans_color_preview, 'HEART_EDGE_COLOR': edge_color_preview}
        self.heart_images = {}
    ...
    def collect_data(self):
        ...
        self.character_data.update_heart_ui_colors(trans_color, edge_color)
