...
class TabBasicSettings(TabBase):
    ...
    def create_widgets(self):
        ...
        # 透過色
        ttk.Label(parent, text="ウィンドウ透過色:", style="Tab.TLabel").grid(row=9, column=0, sticky="w", pady=pady)
        trans_color_frame = ttk.Frame(parent)
        trans_color_frame.grid(row=9, column=1, sticky="ew", pady=pady)
        ...
        ttk.Button(trans_color_frame, text="色の選択", style="Tab.TButton",
                   command=lambda: self.editor.pick_color(trans_color_preview)).grid(row=0, column=1, sticky="w")
        ttk.Button(trans_color_frame, text="画像から色を取得", style="Tab.TButton",
                   command=lambda: self.editor.enter_eyedropper_mode(trans_color_preview)).grid(row=0, column=2, padx=padx)

        # 縁色
        ttk.Label(parent, text="ウィンドウ縁色:", style="Tab.TLabel").grid(row=10, column=0, sticky="w", pady=pady)
        edge_color_frame = ttk.Frame(parent)
        edge_color_frame.grid(row=10, column=1, sticky="ew", pady=pady)
        ...
        ttk.Button(edge_color_frame, text="色の選択", style="Tab.TButton",
                   command=lambda: self.editor.pick_color(edge_color_preview)).grid(row=0, column=1, sticky="w")
        ttk.Button(edge_color_frame, text="画像から色を取得", style="Tab.TButton",
                   command=lambda: self.editor.enter_eyedropper_mode(edge_color_preview)).grid(row=0, column=2, padx=padx)
        ...
        self.widgets = {
            ...
            'SPEECH_FREQUENCY_LABEL': freq_value_label, 'TRANSPARENT_COLOR': trans_color_preview,
            'EDGE_COLOR': edge_color_preview, 'SYSTEM_MESSAGES': {},
